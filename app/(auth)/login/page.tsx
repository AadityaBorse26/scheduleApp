...
function LoginContent() {
  ...
  return (
    <div className="w-full max-w-md p-8 rounded-3xl border border-slate-900 bg-slate-950/60 backdrop-blur-xl shadow-2xl relative z-10 flex flex-col items-center">
      {/* App Emblem */}
      ...
      <p className="text-sm text-slate-400 text-center mb-8 leading-relaxed">
        Coordinate schedules, overlay availability, and sync Google Calendar events with your friends.
      </p>

      {errorMsg && (
        <div className="w-full mb-6 p-4 rounded-xl border border-rose-500/20 bg-rose-500/5 text-rose-400 text-xs leading-relaxed text-center">
          {errorMsg}
        </div>
      )}
      ...
      <div className="mt-8 text-center">
        <p className="text-xs text-slate-500 leading-relaxed">
          By signing in, you grant access to your Google Calendar details as a read-only source to block out busy event timings.
        </p>
      </div>
    </div>
  );
